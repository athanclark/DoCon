...
import Categs 
import DPrelude 
       (PropValue(..), InfUnn(..), lookupProp, lkp, listsOverList, 
        fmapfmap
       )
import SetGroup
import Ring0_  
...
vecBaseSet v dm =  
  (case  
       (vecRepr v, lkp dm Set)  
   of
     (_  , Just (D1Set o)) -> (dm, o)
     (a:_, _             ) -> vset $ snd $ baseSet a eFM
     _                     -> (dm, error "baseSet (Vec []) dom'\n")
  )
  where
  vset s = (addToFM dm Set $ D1Set o, o)
    where
    o =
      let   -- mind that the components of  v  are presumed to 
        ...
        bel' md (Vec xs) = (genericLength xs)==size && bl md xs
                                              where
                                              bl 'r' = all (bel 'r')
                                              bl _   = const True
        list' = case  list  of 
...
vecBaseAddSemigroup v dm =  
  (case  
       (lkp dm AddSemigroup, vecRepr v)
   of
     (Just (D1Smg s), _  ) -> (dm, s)
     (_             , a:_) -> vsmg (zeroS a) $ upAddSemigroup a eFM
     _                     -> 
                      (dm, error "baseAddSemigroup (Vec []) vDom\n")
  )
  where
  vsmg zr domA = case  (lkp domA Set, lkp domA AddSemigroup)  of

    (Just (D1Set setA), Just (D1Smg sA)) -> 
                              (addToFM dm AddSemigroup $ D1Smg s, s)
      where
      s = 
       let
...
vecBaseAddGroup v dm = 
  (case  
       (vecRepr v, lkp dm AddGroup)
   of 
     (_  , Just (D1Group g)) -> (dm, g)
     (a:_, _               ) -> vgr (zeroS a) $ upAddGroup a eFM
     _                       ->
                          (dm, error "baseAddGroup (Vec []) vDom\n")
  )
  where
  vgr zr aDom = case (lkp aDom Set, lkp aDom AddGroup)  of

    (Just (D1Set aS), Just (D1Group aG)) -> 
                                (addToFM dm AddGroup $ D1Group g, g)
      where
      g = 
       let
         zeroV@(Vec zrs)     = constVec v zr
         ...
         props' = 
           if (vecSize v)==1 || (osetCard aS)==(Fin 1) then props_aG
             ...
             [(IsCyclicGroup    , No ), 
              ...
              (IsPrimeGroup     , No ),
              (IsOrderedSubgroup,
                               lookupProp IsOrderedSubgroup props_aG
              )
             ]
       in
...
vecBaseMulSemigroup v dm =  
  (case 
        (vecRepr v, lkp dm MulSemigroup)
   of
     (_   , Just (D1Smg s)) -> (dm, s)
     (e:xs, _             ) ->
                    mulsmg (e:xs) (unity_m e) $ upMulSemigroup e eFM
     _                      ->
                      (dm, error "baseMulSemigroup (Vec []) vDom\n")
  )
  where  
  mulsmg xs un_m eDom = 
                     case  (lkp eDom Set, lkp eDom MulSemigroup)  of
 
    (Just (D1Set eSet), Just (D1Smg eH)) -> 
                              (addToFM dm MulSemigroup $ D1Smg s, s)
     where
     s = 
      let
        size             = vecSize v
        ...
        unV'_m = case un_m  of  Just u -> Just $ Just $ constVec v u
                                _      -> Nothing
        ------------------------------------------------------------
        gens' = case  eHgens  of     -- optimizations possible 

          Nothing -> Nothing
          Just gs -> case  un_m  of
                       Nothing -> 
                              Just $ map Vec $ listsOverList size gs
...
vecBaseMulGroup v dm = 
  (case  
       (vecRepr v, lkp dm MulGroup)
   of 
     (_  , Just (D1Group g)) -> (dm, g)
     (e:_, _               ) -> gr (unity e) $ upMulGroup e eFM
     _                       ->
                          (dm, error "baseMulGroup (Vec []) vDom\n")
  )
  where
  gr un eDom =  case  (lkp eDom Set, lkp eDom MulGroup)  of

    (Just (D1Set eS), Just (D1Group eG)) ->
                                (addToFM dm MulGroup $ D1Group g, g)
     where
     g = 
      let
        ...
        props' = 
          if (vecSize v)==1 || (osetCard eS)==(Fin 1)  then props_eG
            ...
            [(IsCyclicGroup    , No ), (IsMaxSubgroup,No), 
             ...
             (IsOrderedSubgroup,
                               lookupProp IsOrderedSubgroup props_eG
             )
            ]
      in
      Subgroup 
...
vecBaseRing v dm = 
  (case  
       (vecRepr v, lkp dm Ring)
   of 
     (_   , Just (D1Ring r)) -> (dm, r)
     (e:xs, _              ) -> 
                      rg (e:xs) (zeroS e) (unity_m e) $ upRing e eFM
     _                       ->
                              (dm, error "baseRing (Vec []) vDom\n")
  )
  where
  rg xs zr un_m eDom = case  (lkp eDom Set, lkp eDom Ring)  of

    (Just (D1Set eS), Just (D1Ring eR)) -> 
                                     (addToFM dm Ring $ D1Ring r, r)
     where
     r = 
       let
         zrs            = map (const zr) xs
