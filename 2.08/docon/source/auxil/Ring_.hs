...
where   
import Data.FiniteMap (mapFM, addToFM)

import List      (genericLength, transpose                         )
import Ratio     (numerator, denominator                           )
import DPrelude  (PropValue(..), Z,eFM, fmapfmap, lkp, showsWithDom)
import Categs  
import SetGroup
import Ring0_
    ...
    isomOp (WithPrimeField, wp) = case  frobenius wp  of
      ...
      (fr,frInv) -> 
        (WithPrimeField,
         WithPrimeField' 
          {frobenius            = 
                             (f .fr .fInv, fmapfmap f .frInv .fInv),
...
eucIdeal :: (FactorizationRing a,EuclideanRing a) 
  ...
  {-
   ...
   mode = bMode++eMode++fMode   should be a Substring of  "bef";
...
eucIdeal mode b bs es ft =  
  let
    ...
    bsNew =  
      if  not bMode || not (null bs)  then  bs
        ...
        let  ft'  = if  not (null ft)  then  ft  
             ...
             ft'' = [p^k | (p,k) <- ft']
        in
        if  cEuc==Yes && (genericLength ft'') > 1  then  
                                                 map canonicRem ft''
        else  ft''

    mapFactorization ps =
                if  cEuc==Yes && (genericLength ps > 1)  
                                then  [(canonicRem p,j)| (p,j)<- ps]
                else  ps
    ...
    ----------------------------------------------------------------
    msg = ("eucIdeal_ft mode b bs es ft,"++) .showsWithDom b "b" "R"
    esNew =  
  ...
  else    error $ msg "\nmode  should be a substring of \"bef\" \n"
...
instance Ring Integer  
  ...
  baseRing _ dm = case  lkp dm Ring  of

    Just (D1Ring r) -> (dm, r)
    _               -> (addToFM dm Ring (D1Ring r), r)
      where
...
instance GCDRing Integer  
  ...
  hasSquare _ = 
    error ("hasSquare (n :: Integer):   not done, so far.\n"++
          "It can be replaced with  any ((> 1) . snd) $ factor n \n"
          )
  toSquareFree _ = 
        error ("toSquareFree (n :: Integer):   not done, so far, "++
               "sorry.\nDo it via  (factor n) \n"
              )

  baseGCDRing _ dm = case  lkp dm GCDRing  of

    Just (D1GCDR t) -> (dm, t)
    _               -> (addToFM dm GCDRing $ D1GCDR t, t) 
      where
      t = GCDRingTerm 
