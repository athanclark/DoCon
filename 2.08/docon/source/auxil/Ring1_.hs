...
import DPrelude  (PropValue(..), Z, eFM, lkp, showsWithDom)
import Categs    
...
gcd_test d x y = 
  let 
    (zr,un,rR) = (zeroS d, unity d, snd $ baseGCDRing d eFM)
...
instance LinSolvRing Integer  
  ...
  baseLinSolvRing _ dm = case  lkp dm LinSolvRing  of

    Just (D1LinSolvR t)-> (dm, t)
    _                  -> (addToFM dm LinSolvRing$ D1LinSolvR t, t)
      where                      
      t = LinSolvRingTerm 
            {linSolvRingProps =
...
instance EuclideanRing Integer  
  ...
  baseEucRing _ dm =  case  lkp dm EuclideanRing  of

    Just (D1EucR t) -> (dm, t)
    _               -> (addToFM dm EuclideanRing $ D1EucR t, t)
      where
      t = EucRingTerm {eucRingProps =
