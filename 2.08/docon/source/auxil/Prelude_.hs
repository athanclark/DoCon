...
module Prelude_   
  ...
  (Cast(..), ct, ctr, 
   ...
   lkp, eFM, zipRem, allMaybes,  mapmap, fmapmap, mapfmap, fmapfmap,
   ...
   updateProps, mbPropV, lookupProp, addUnknowns, 
   foldlStrict, foldl1Strict,  
   antiComp,  minBy, maxBy, minimum, maximum, minAhead, maxAhead
  ) 
where   

import Data.FiniteMap (FiniteMap, emptyFM, lookupFM)
...
lkp :: (Ord a) => FiniteMap a b -> a -> Maybe b
lkp = lookupFM
...
foldl1Strict f (x:xs) =  foldlStrict f x xs
foldl1Strict _ []     =  error "foldl1Strict _ [] \n"
