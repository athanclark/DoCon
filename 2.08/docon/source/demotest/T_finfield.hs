...
import Categs     (CategoryName(..), Domain1(..), Subring(..),
                   ResidueE(..)
                  )
...
t_finfield_ =
  let  
    ...
    r0            = Rse 0 iI dZ 
    [r1,r2,r3,r4] = map (ctr r0) [1..4::Z]   -- project to residues
                                       
    d5      = upField  r0 eFM     -- full description for Z5
    ...
    dX     = upEucFactrRing uX eFM          -- X = Z5[x]
    (_,rX) = baseRing uX dX
    ...
    dF = upField u eFM                    -- F25 = Z5X/(p) 

    rF = case  lookupFM dF Ring           -- another way to get 
         of                               -- ring description
           Just (D1Ring r) -> r
           _               -> error "Ring not found in F25\n"
  ...
  in  
  ([divTest,x8Check,powCheck,gcdCheck],  
   gcd12, 
   [propsX,propsF],
   (p, [u,x'], powers, f1, f2)
  )                         
