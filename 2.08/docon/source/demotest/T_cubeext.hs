...
cubicExt :: (Field k, FactorizationRing (UPol k)) 
  ...
  --
  -- Example:  let  { un = 1:/1 :: Q;  dK = upField un eFM }
...
cubicExt a b dK =  
  let
    ...
    unK1    = Rse unA ddI dA   
    dK1     = upField unK1 eFM    -- domain of  K1 = k(d)
    ...
    dB          = upGCDLinSolvRing unB eFM  -- domain B
    ...
    (dI,iRad) = gensToIdeal gs [] [(IsGxBasis,Yes)] 
                                           [(IsMaxIdeal,Yes)] dB eFM
    unE     = Rsi unB (iRad,dI) dB
    kToE    = ct unE .kToB
    dE      = upField unE eFM            -- E = B/iRad = K1(u,v,r)
...
t_cubeext_ =       -- Example. Build the extension of Q = Fraction Z
  let              --          with  f = t^3 - t + 1
    ...
    [x,y,z]          = roots 
    Just (D1Ring rE) = lookupFM dE Ring   -- look into ring E
    ...
    n1     = fromi x 1                     -- 1 in E
    fRoots = [x^3 -x +n1| x<- roots]  --it should be [0,0,0], 0 of E
    ...
    relCol = transp $ Mt [rels] eFM   -- to print matrix nicely 
