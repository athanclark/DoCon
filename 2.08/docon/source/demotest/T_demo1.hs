...
This program 
  ...
  reads (parses) from the string the polynomials  f,g  in variables 
    ["x","y"] with the coefficients from R,
  evaluates  gcd(f,g).
--------------------------------------------------------------------
-}
...
where
import DPrelude   (PropValue(..), Z, ctr, eFM, smParse, mapmap    )
...
t_demo1_ =
  let
    ...
    unQ = 1:/1  :: Q
    zrQ = zeroS unQ
    dQ  = upField unQ eFM         -- domain term of Q
    mM  = [[1,2,3,4,5,6],
           [5,0,6,7,1,0],
           [8,9,0,1,2,3]
          ]                :: [[Z]]
    ...
    dR = upField r1 eFM         -- the full domain description
    ...
    gcdFG = gcD [f,g]
           -- this should be (x+1)^2 because x^5+1 = (x+1)^5  over R

    gcdCheck = gcdFG==((x+p1)^2)
    checks   =
      [ker==checkKer, checkInvM', rFieldCheck, check2by3R, gcdCheck]
  in
  (checks, (ker,iM'), (rProps,q,gcdFG))
