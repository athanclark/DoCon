...
import DPrelude   (InfUnn(..), Z, eFM, ctr, product1, sort, 
                   alteredSum, smParse
                  )
import Categs     (Dom(..), OSet(..), ResidueE(..), Factorization)

import SetGroup   (Set(..), unity, times, invertible, divides,
                   eqFactrz, unfactor, gatherFactrz
                  )
import RingModule 
       (Ring(..), CommutativeRing(), GCDRing(..), Field(), 
...
pFactorizControlSum :: (CommutativeRing a, PolLike p, Ring (p a)) 
                       =>
                       Factorization (p a) -> ((a,[Z]), Z)

  -- Also usable for timing:
  -- small output forces the factorization evaluation

pFactorizControlSum pairs = 
  let 
    muls             = map snd pairs
    (ps,ps')         = unzip $ map (pControlSum .fst) pairs
    su (a,js) (b,ks) = (a+b, zipWith (+) js ks)
  in
  (foldl1 su (ps++ps'), alteredSum muls)
                         



--------------------------------------------------------------------
testFFPolFactrz :: (Field k) =>  Z -> Factorization (UPol k) -> Bool
                              -- n    ftn
  ...
  -- factorization enumerates all the irreducible normed polynomials
  -- of degree  d  - see [La], chapter 7, exercises 1,2.
  -- So, the main part of the test is to form the list  group(d)  of
  -- the factors in  ftn of degree  d  and verify that its length is 
  -- numOfNPrimesOverFin(q,_).

testFFPolFactrz n ftn =  
  if
     n < 1 || null ftn  then
             error ("testFFPolFactrz n ftn: \n"++
                    "n > 0 and non-empty factorization ftn needed\n"
                   )
  else
    let
      ...
      q = case  osetCard sC  of
            Fin n -> n
            _     -> error ("testFFPolFactrz _ ftn:  field"++
      ...
      testGroups []     = True
      testGroups (d:ds) = 
             (genericLength ds')==(numOfNPrimesOverFin q $ factor d)
             && (testGroups ds'')    where
                                     (ds',ds'') = span (== d) (d:ds) 
    in
...
t_pfactor1_ q =    
             -- Factoring univariate polynomial over a finite field.
             -- Prime  q  required.
  let
                                     -- prepare  K = Z/(q), T = K[t]
    iI = eucIdeal "bef" (q::Z) [] [] []
    ...
    r0 = Rse 0 iI dZ
    [r1,r2,r3,r4,r5,r6,r7,r8,r9] = map (ctr r0) [1..9::Z]
    dK  = upField r0 eFM
    unT = cToUPol "t" dK r1  :: T
    dT  = upEucFactrRing unT eFM
    t   = varP r1 unT        -- t <- T
  ...
  -- in  factor (x^6 + m2*(tt^2 -m1)*x^3 + m3*tt)
...
testFactorBi :: 
          FactorizationRing p => 
          p -> String -> String -> Z -> Z -> (Bool, Factorization p)

       -- Set different primes  f, g  as the strings to parse by the
       -- given sample, then factor  f^n*g^m ...
       -- Example:   
       -- testFactorBi unP "x^3 + y*(x+1)" "x^3 + y*(x^2*y + 2)" 1 1

testFactorBi smp fS gS n m = 
       let 
...
t_pfactor2_ q =  
  let
    iI         = eucIdeal "bef" q [] [] []
    r0         = Rse 0 iI dZ 
    [r1,r2,r3] = map (ctr r0) [1, 2, 3::Z] 
    dK         = upField r0 eFM
    vars       = ["x","y"]
    o          = lexPPO 2
    unP        = cToPol o vars dK r1 
    [x,y]      = varPs r1 unP               -- x,y in P
    kInP       = map (times unP) [0..(q-1)]  
    y0:y1:y2:_ = kInP
    kBut0      = tail kInP    -- k\{0}
    ----------------------------------------------------------------
    ...
    isAlmostCorrect (f,ftn) = f==(product1 [p^n | (p,n) <- ftn])
    ...
    -- Depends on  n,m <- Z.  List of triplets (f, g, factor (f*g)),  
    ...
    randRandFactor    n    m   = 
        [((f*g)==unfactor ft, f, g, ft) | (f,g,ft) <- randTrips n m]
  in
  (unP, family1, randRandFactor)
