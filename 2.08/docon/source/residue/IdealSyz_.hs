...
module IdealSyz_  
  ...
  -- Tools for setting  Ideals and Subrings.
  --
...
import DPrelude   (InfUnn(..),PropValue(..), lookupProp,updateProps)
import Categs     
...
gensToIdeal igens fts bProps iProps dR dm = 
  let
    Just (D1Set setR) = lookupFM dR Set 
    ...
    nonprimaryItem fs = (genericLength fs) > 1
    ...
    max' = case  (maxI,prime',pir)  of

      (No , _  , _  ) -> No
      (Yes, No , _  ) -> 
                   error $ msg ("(IsMaxIdeal,Yes)  from iProps "
                                ++"contradicts deduced (Prime,No)\n"
                               )
      (_  , No , _  ) -> No
      (Yes, _  , _  ) -> Yes
      (_  , Yes, Yes) -> Yes
    ...
    msg = ("gensToIdeal generators fts bProps iProps dR dm,"++)
          .("\ndR         = "++) .showsDomOf e 
          .("\nbProps     = "++) .shows bProps
          .("\niProps     = "++) .shows iProps
          .("\ngenerators = "++) .shows igens  .("\n\n"++)
    ...
    (dm5,_) = 
             idealGrSemigrToSubring' propsR aGrI mSemigrI [] dR2 dm4
    ----------------------------------------------------------------
    iI = case  (igens', reducedUn==zr) 
         of
           ([], _   ) -> zeroIdeal iProps rR   
           (_ , True) -> unityIdeal un rR 
           (_ , _   ) -> Ideal {idealGens     = Just igens',
                                idealProps    = iProps',
  ...
  case  detaching
  of
    Yes -> (addToFM dm5 IdealKey $ D1Ideal iI, iI)
    _   -> 
...
idealSetToAddSubsemigroup' zr givenProps dR dm = 
  ...
  s =
   ...
   in
   Subsemigroup 
            {subsmgType    = Add,      subsmgUnity = Just $ Just zr,
...
idealGensToAddSubgroup' zr givenProps gens dR dm =
  let
    ...
    Just (D1Set setH) = lookupFM dm Set
...
ringAddSubgroupToMulSubsemigroup'
                              zr setH  _       _  givenProps dR dm =
                                      --smg_aH agH
  let
    ...
    s = Subsemigroup 
                {subsmgType    = Mul,    subsmgUnity = Just Nothing,
...
idealGensToSubset mode givenProps gens dR dm =
  ...
  let
    ...
    propsSetR  = osetProps setR
    (dR2,syzT) = baseLinSolvRing zr dR1 
    ...
    list' = Nothing      -- so far 
                         -- In some cases, we might list the ideal
    os = OSet {osetSample  = zr,
               membership  = bel', 
               osetCard    = card', 
               osetPonited = Just (Just zr), 
               osetList    = list',
  ...
  in
  case  (gens', detaching)
  of ([], _  ) -> (addToFM dm Set $ D1Set set0, set0)  
    (_ , Yes) -> (addToFM dm Set $ D1Set os, os  )    
