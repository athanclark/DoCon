...
import DPrelude 
       (InfUnn(..), PropValue(..), Expression(..), Z, lkp, ct, ctr, 
...
instance (EuclideanRing a, Random a) => Random (ResidueE a)  
  ...
  randomR (l,h) g = (ctr l a, g')  
                    where  (a,g') = randomR (resRepr l, resRepr h) g
...
ifCEuc             x      name      msg                   v =
  case
      lkp (dom x) EuclideanRing  
  of
    Nothing         -> 
      error$ msg $ ("\nEucRingTerm not found in  "++) $ (name++"\n")

    Just (D1EucR t) -> 
          case  isCEucRing t  of
            Yes -> v
            _   -> error $ msg
...
instance (EuclideanRing a, 
  ...
  baseLinSolvRing r dm = case  (lkp dm LinSolvRing, dom r)  of
    ...
    (_                  , aD) -> ResEuc1_.ifCEuc_rse_ aD dm msg (dm',t)
      ...
      msg = ("baseLinSolvRing r dom',"++) .showsWithDom r "r" ""
            .('\n':)
      dm' = addToFM dm LinSolvRing $ D1LinSolvR t
      t   = LinSolvRingTerm 
             {linSolvRingProps =
...
instance (EuclideanRing a, 
  ...
  baseGCDRing  r@(Rse _ iI aD) dm =  case lkp dm GCDRing  of
    ...
    _              -> ResEuc1_.ifCEuc_rse_ aD dm msg $ 
      ...
      dom' = addToFM dm GCDRing $ D1GCDR t
      t    = GCDRingTerm 
...
instance (EuclideanRing a, 
  ...
  baseFactrRing  r@(Rse _ iI aDom) dom =  
                                  case lkp dom FactorizationRing  of
...
instance (EuclideanRing a, 
  ...
  baseEucRing  r@(Rse _ iI aDom) dom = case lkp dom EuclideanRing of
...
instance Set (ResidueE Z)                     -- specializes baseSet
  ...
  baseSet r dm =  case  (lkp dm Set, pirCIBase $ resPIdeal r)  of

    (Just (D1Set o), _) -> (dm, o)
    (_             , b) -> (addToFM dm Set $ D1Set o, o)
      where
      o = 
        let
          bel 'r' r = (pirCIBase (resPIdeal r))==b && isCorrectRse r
          bel _   _ = True
        ...
        OSet {osetSample  = r,          membership  = bel, 
              osetCard    = Fin b,      osetPointed = Just (Just r),
              osetList    = Just list,
...
instance AddSemigroup (ResidueE Z)   -- specializes baseAddSemigroup
  ...
  baseAddSemigroup r dm = case  lkp dm AddSemigroup  of
...
instance AddGroup (ResidueE Z)
  where
  baseAddGroup r dm =
               case  (lkp dm AddGroup, pirCIFactz $ resPIdeal r)  of

    (Just (D1Group g), _ ) -> (dm, g)
    (_               , ft) -> (addToFM dm AddGroup $ D1Group g, g)
      where
  ...
  divide_m r1 r2 =             -- applies special extended gcd for Z
    ...
    let                        -- Z/(b) =iso= Z/(b1) +..+ Z/(bn)
      ...
      div1 b x y = let  (v,d) = gcdE_mod_b b 0 1 b y 
                   ...
                   case 
                       quotRem x d of  (q,0) -> Just $ rem (v*q) b
                                       _     -> Nothing
...
instance Ring (ResidueE Z)                   -- specializes baseRing
  ...
  baseRing r dm = case  (lkp dm Ring, resPIdeal r)  of
