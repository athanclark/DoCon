...
where
import Data.FiniteMap (FiniteMap, lookupFM, addToFM, mapFM,
                       fmToList, listToFM
                      )
import Maybe      (fromMaybe                                       ) 
import List       (genericLength, nub                              )
import DPrelude   (Z, allMaybes, sortBy, minBy, showsWithDom       )
...
import EPol0_     (epolMons, eLpp, epolLCoord, epolECp             )
...
--
-- DIFFERENCE:
-- in  updatePairs,  we do not find so totally reduced D1'  as the
-- paper does 
-- (by the way, the paper typos in this place badly, so that D1' 
--  always occurs empty !).
-- Instead, we apply the  "mutually prime", F- and M- criteria  
-- - which agrees with the air of the paper.
--
-- And we join F- and M- criteria into one  f_m_criterion  which 
-- simply removes the Tit -s multiple for the other Ti't -s -
...
-- type FinMap_ a = FiniteMap Z (Pol a,Bool)

type Row           = [(Z,PowerProduct)]
type CriticalPairs = [(Row,Z)]

--------------------------------------------------------------------
f_No_ :: Set a => FiniteMap Z (a,Bool) -> Z -> (a,Bool)   
        
                                         -- tab-> i-> (f(i),isRedex)
f_No_ tab i = case  lookupFM tab i  of  
                                   Just x -> x 
                                   _      -> error $ msg $ msg' "\n"
  where
  msg      = ("gBasis fs   for the list of PolLike things ..."++)
             .("\n\nf_No_ table "++) .shows i
             .(":   index not found in  table."++)
             .("\ntable  contains the items  (j, (f,bool)) "++) 
             .("with  (j,bool) <- \n  "++) .shows tab'List
  tabList  = fmToList tab
  tab'List = [(i,b) | (i,(_,b)) <- tabList]
  msg'     = case  tabList  of 
              
    []          -> id
    (j,(f,b)):_ ->
             ("\nHead item is  (j,(f,b)),  (j,b) = "++) .shows (j,b)
             .showsWithDom f "f" ""
...
cnAs_ev_                   (f,fs)   =  
  if 
    ...
    in 
    if  a==(canAssoc a)  then  (f,fs)
    else                       (cPMul ii f, map (cPMul ii) fs)
--------------------------------------------------------------------
redCan_ev_ mode (Just cnAs) gs f = cnAs $ polNF_ev mode gs f
redCan_ev_ mode _           gs f =        polNF_ev mode gs f
   -- 
   -- In the field case, this is always called with (Just cnAs_ev_).
   -- In Euclidean case, it is called with  Nothing  in the 
...
redToStableLpps_ev_ mbCnAs fs = rtos fs $ map (eLpp .fst) fs
  ...
  rtos fs lpps = let  fs'   = reverse $ redOnePath_ev_ "" mbCnAs fs
                 ...
                 in
                 if  lpps'==lpps  then  fs'  else  rtos fs' lpps'

--------------------------------------------------------------------
minimalPair :: PPComp -> CriticalPairs -> ((Z,Z),PowerProduct)
               -- cp     dD                (i,j) tTij
...
minimalPair cp dD = 
           ...
           in 
           minBy l2 [((i,j),p) | ((i,p),j) <- dD']


-- blocks -> (i,j,o)   such that (o,Tij)  is  ecp-minimal  of  Tij

selectPair_ev ecp indBlocks =
  let
      indBlockMin (block,o) = (ij,(o,tT))
                            where  (ij,tT)  = minimalPair cp_o block
...
--
-- We skip the type, for this has to work for dD in the Euclidean 
-- case too.
-- :: ..=> CriticalPairs -> Z -> Z -> CriticalPairs

removePair_ []              _ _ = []
removePair_ ((row,j'):rows) i j =        
  if
...
-- removePair_ev_ indBlocks i j o --> indBlocks'
--
-- It also removes the row if the row turns empty; the same is with
-- the block.

removePair_ev_ []             _ _ _ = []
removePair_ev_ ((b,o'):indBs) i j o = 
      if
        o'==o  then  case  removePair_ b i j  of  [] -> indBs
                                                  b' -> (b',o):indBs
...
--------------------------------------------------------------------
-- Remove from  dD  all the  pairs  (i,Tij)  such that 
-- p divides Tij  and  T_p_i /= Tij  and  T_p_j /= Tij.
--
...
{-
This is a commentary. The real function is  updatePairs_ev:
see below.

updatePairs :: FinMap_ a -> CriticalPairs -> Z -> CriticalPairs
...
repeatUpdatePairs_ev  applyPPMutPrime indBs fsM =  
                       foldl
                          (updatePairs_ev applyPPMutPrime fsM) indBs
...
{-
This COMMENTARY shows the method for the computing of a G-basis 
over a field.
    ...
    processPairs fsM dD rR gG =  
      ...
      if isZero h  then  processPairs fsM (removePair_ dD i j) rR gG
        ...
        let  rR'     = rR+1
             ...
             markRedex _ (f,redt) =  (f, redt || p_redex f)

             fsM'  = addToFM fsM rR' (h,False)
             dD'   = updatePairs fsM' dD rR'
             fsMr  = mapFM markRedex fsM
             fsM'' = addToFM fsMr rR' (h,False)
          in
...
-- blockwise.
-- The "mutually prime" criterion is not valid if we have more 
-- than one coordinate.
--
...
-- vector parts are "parallel" (with the same coefficients) to the 
-- ones on polynomials.
...
gBas_field_ev_  fs =  
    ...
    processPairs singleC fsM indBlocks rR gG =  
      let
        ...
        (h',_,_) = 
          fromMaybe (error $ messgGBasEV_ fs "gBas_field_ev_" msg) $
                    {- _scc_ "sp"$ -}   sEPVecP fi fj

        h   = {- _scc_ "redCan_ev_"$ -}  redCan_ev_ "" justCnAs gG h'
      ...
      if  isZero $ fst h  then                                  
        ...
        let 
          ...
          o_p_redex (g,_) =  o'==o && ppDivides p q
                                              where  (o',q) = eLpp g
          markRedex _ (f,redt) = (f, redt || o_p_redex f)
  ...
  of
    ...
    fs  -> 
      let  
        ecp = epolECp $ fst $ head fs
        r   = genericLength fs           
        fsM = listToFM $ zipWith (\i f->(i,(f,False))) [1..r] fs
...
messgGBasEV_                   fs@(f:_)     funcName =
  ...
  .showsWithDom (fst f) "fst $ head fs" "" 
...
isGBas_field_ev_  fvs = case  redToStableLpps_ev_ justCnAs fvs  of
  ...
  fvs -> 
    let 
      r   = genericLength fvs  
      fsM = listToFM $ zipWith (\i fv->(i,(fv,False))) [1..r] fvs
