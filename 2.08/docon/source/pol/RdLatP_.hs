...
import DPrelude   (Z, compBy, sum1, maximum, maxBy, compBy, mergeBy,
                   sortBy
                  )
...
reduceLattice_UPolField bnd bs = 
  reduceLattice_UPolField_special 
           bnd [] 
           (sortBy (compBy snd) [(b,ord b) | b <- bs, not $ null b])
                       where
                       ord [] = -1
...
reduceLattice_UPolField_special :: 
  ...
  -- It is separated because the polynomial factorization calls
  -- directly for this function, providing respectively large  b's  
  -- part.
...
reduceLattice_UPolField_special bnd b's bs =
  ...
  where
  (rs,pmt)    = rd (genericLength b's) b's bs []
  pmt'        = reducePmt pmt
    ...
    if
      ...
      of
        ...
        (_ , True) ->
              ...
              in   
              rd (genericLength lts) lts ((b'',ob''):(gts++bsT)) pmt
    ...
    if  i > k  then  uR++((i,f):u)
      ...
      of
        ...
        (v:vs', _   ) ->
           ...
           in  
           rd (v:vs') (addVect uR vl') (addVect ((i,f):u) vr') (i-1)

  addVect u v = sumSimilar $ mergeBy (compBy fst) u v  
    where                                          
    sumSimilar []         = []
    sumSimilar ((i,f):ps) = let (ps',ps'') = span ((==i).fst) ps
                                s          = sum1 (f:(map snd ps'))
                            in
