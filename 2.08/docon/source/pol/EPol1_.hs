...
module EPol1_ 
  ...
  -- Extended polynomial operations. Continuation.
  --
...
import List    hiding (sortBy, maximum) 
import Prelude hiding (maximum        )

import DPrelude   (Z, maximum,mergeBy, ct,ctr, sortBy, showsWithDom)
import Categs     (Dom(..)                                         )
...
import Pol__      (sPol                                            )
import EPol0_   
       (EPol(..), EPPOTerm, EMon, eppoMode, eppoWeights, epolMons,
        epolPol, epolLCoord
       )
...
polEPolMul                           f        g      = 
  ...
  pm (m:ms) (n:ns) = (emonMul z m n)++mons
                       where
                       mons = epolMons ((mEPolMul m $ ct g ns) +
                                        (ct g $ pm ms (n:ns))
                                       )
...
epolToVecPol :: (CommutativeRing a) => Z -> EPol a -> Vector (Pol a)
  ...
  --      eo  = (eAntiLex, "a", [], _)
  -- in
  -- epolToVector 6 f --> Vec [5*x+4, 5*x^2*y+4*x, 0, y, 0, 0]
  ...
  let
    ...
    lastZeroes =  
      if 
        n < maxCoord  
            then  
              error $ ("epolToVecPol n f,"++) $ 
                      ("\nn = "++) $ shows n $ showsWithDom f "f" ""
...
vecPolToEPol ::
  ...
  -- The monomials of each  fi  extend with the coordinate number i, 
  -- are sorted under  eo  (this is skipped if eo shows `agreed'), 
  -- and merged to the previously accumulated (eo-ordered)
  -- e-polynomial.
...
vecPolToEPol eo (Vec []) = 
  ...
  in
  error $ ("vecPolToEPol eppOrdTerm (Vec []),"++) $
       (("\neppOrdTerm = (<eppComp>,"++m++",weights,<ppComp>)")++) $
        ("\nweigths =  "++) $ shows wts "\n\nEmpty vector\n"

vecPolToEPol eo (Vec fs) =  
  let
    (ecp,mode,_,_)  = eo
    (f,n)           = (head fs, genericLength fs)
...
sEPol :: (GCDRing a) => EPol a-> EPol a-> Maybe (EPol a,Mon a,Mon a)
  ...
  --   Nothing               when the leading coordinates of  f,g  
  --                         differ,
...
sEPVecP (f,v1) (g,v2) = case  sEPol f g  of
...
{- reserve **********************************
instance(Convertible a b, AddGroup b) => 
                                   Convertible (EPol a) (EPol b)
  where
  cvm (EPol mons _ pol) (EPol _ eo' pol') =
    case (cvm pol pol', sample pol')
    of(Nothing, _) -> Nothing
      (_      , b) -> Just (reo (EPol (cvMons mons) eo' pol'))
                            where reo    = reordEPol eo'
                                  cvMons = map (\ (a,p)->(cv a b,p))
...
instance (CommutativeRing a,Ring b,Convertible a b) => 
                               Convertible (EPol a) (Vector (Pol b))
  where
