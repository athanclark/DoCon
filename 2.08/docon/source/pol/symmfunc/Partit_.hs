...
where
import List     (genericLength, genericSplitAt, genericReplicate)
import DPrelude (Z, Comparison, sort                            )
...
fromEPrtt []     = []
fromEPrtt (j:js) = (j, 1+(genericLength js')): (fromEPrtt ks)
                                            where  
                                            (js',ks) = span (==j) js
...
conjPrtt []         = []        
conjPrtt [(j,m)]    = [(m,j)]
conjPrtt ((j,m):pt) = let j1  = fst $ head pt
                          pt1 = [(j'+m,m') | (j',m') <- conjPrtt pt]
                      in  pt1++[(m,j-j1)]
--------------------------------------------------------------------
prttUnion :: Partition -> Partition -> Partition
  -- repeated diagram lines are copied, say, 
  --                                      [3,2,1] [3*2] -> [3*3,2,1]
prttUnion = u   
  where
  u []        q           = q
...
randomEPrtts randoms w = case  (w, w < 0)  of
