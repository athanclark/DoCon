...
where
import Data.FiniteMap (mapFM, addToFM, listToFM)  

import Maybe       (fromMaybe)        
import List hiding (sortBy)

import DPrelude   (Z,PropValue(..), allMaybes,minBy, sortBy, mapmap)
import Categs     (Property_GCDRing(..), GCDRingTerm(..)           )
import SetGroup   (MulSemigroup(..), isZero, unity, divides        )
import RingModule (CommutativeRing(), EuclideanRing(..), 
                   GCDRing(..), eucGCDE
                  )
...
monDivides                        (a,p)    (b,q) = 
                                        ppDivides p q && divides a b
--------------------------------------------------------------------
minimalPair :: (Eq a) => PPComp -> CriticalPairs a -> ((Z,Z),Mon a)
                                                    --  i j  tTij
minimalPair              cp        dD =     -- tTij is minimal by cp
  let
    l2 (_,mon1) (_,mon2) = cp (snd mon1) (snd mon2)
  in
...
minimalPair_ev ecp indBlocks =
  let         
      ...
      l2 (_,p) (_,q) = ecp p q
      ((i,j),(o,_))  = case  map indBlockMin indBlocks  of
...
monMutPrimeInv (a,p) (b,q) = 
              ppMutPrime p q && (eucNorm a)==unN && (eucNorm b)==unN
                                             where
                                             unN = eucNorm $ unity a
  ...
  let
    ...
    row0  = 
      if  not applyPPMutPrime  then  row'o
        ...
        [x| x <- row'o, not $ monMutPrimeInv mon $ lm $ fst $ snd x]
    row'' = [(i,tTit f) | (i,f) <- row0]
                                   where  tTit = monLcm mon .lm .fst
...
gbas_ev_  fs =  
    ...
    processPairs singleC fsM indBlocks r gs =  
      let
        ...
        (h',_,_) =
               fromMaybe 
                 ...
                 {- _scc_ "sp_euc"$ -}  sEPVecP fi fj

        h   = {- _scc_ "polNF_ev_euc"$ -}  polNF_ev "" gs h'
      ...
      if  isZero $ fst h  
        ...
        let  
          ...
          markRedex _ (f,redt) = (f, redt || eLmRedex f)

          fsM'       = addToFM fsM r' (h,False)
          indBlocks' = updatePairs_ev  singleC fsM' indBlocks r'
          fsMr       = mapFM markRedex fsM
          fsM''      = addToFM fsMr r' (h,False)
        in
  ...
  of
    ...
    fs  -> 
      let  
        EPol _ eo pol  = fst $ head fs
        ecp            = eppoECp eo
        Pol _ c _ _ aD = pol
        (_,gcR)        = baseGCDRing c aD
        (props, r)     = (gcdRingProps gcR, genericLength fs)
      ...
      in
      case  lookup WithCanAssoc props  of 
                                        Just Yes -> map cnAs_ev_ gs'
                                        _        -> gs'
