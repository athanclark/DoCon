...
import DPrelude   (InfUnn(..), Z, eFM, sum1, ct, ctr, mapmap, 
                   showsWithDom
                  )
...
extendFieldToDeg :: 
  ...
  -- sDom  is the domain description for  s,  setting it with  eFM
...
extendFieldToDeg s sDom d =
  if
    ...
    let  p:_ = dropWhile ((< d).deg) $ primes s
     ...
     in  (unF, upField unF eFM)

--------------------------------------------------------------------
det_upol_finField :: (Field k) => 
                         [ResidueE (UPol k)] -> [[UPol k]] -> UPol k
                         -- ext                 mM   
...
det_upol_finField ext mM = 
  let
    ...
    unT      = cToUPol "t" dK un
    dT       = upEucRing unT eFM     -- T = k[t],  F = T/(p)
    (unF,dF) = case  (ext, cardK)  of 

            (e:_, _    ) -> (unity e, upField e eFM)
    ...
    interpTab zr' extList frobe mM = 
      let
        ...
        takeEnough n (o:orbs) =
          if 
            n < 1  then  []
          else         let  lo = genericLength o
                       in   if  n <= lo  then  [genericTake n o]
