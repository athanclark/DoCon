...
import List     ((\\), partition, transpose, genericLength,
                 genericSplitAt
                )
import Prelude  hiding(maximum)
import DPrelude (Z, eFM, sum1, maximum, minBy, maxBy, sortBy, ct, 
                 ctr, mapmap, showsWithDom
                )
...
import ResEuc_  ()
import RsePol_  () 
import UPol_    (PolLike(..), UPol(..), upolMons, deg0, lc, varP,
                 cPMul, cToUPol
                )
import Pol__    (RPolVar)
import RPol_    (RPol(..), RPol'(..), rpolRepr, rpolVComp,
                 rpolHeadVar, rpolVars, cToRPol, varToRPol, 
                 substValsInRPol 
                ) 
import RPol0_   () 
...
henselLift f h1 g1 mbV1 p pp n m = 
  let
    ...
    repeatBy1 n (h,g,v,vR,pp') = 
                         ...
                         else   repeatBy1 (n+1) $ lift1 h g v vR pp'
...
henselLift_test p f h g pairs = map test $ zip (ppowers (p^2)) pairs
  where
  ppowers pp = pp:(ppowers (p*pp))
  test (pp,(h,g)) = let  opp = eucNorm pp
                    in
                    isJust (pCDiv (f-h*g) pp)
                    && 
                    order h < opp  &&  deg h == dH  &&  lc h == un
                    &&
                    order g < opp  &&  deg g == dG 
...
henselLift f h1 g1 hc1 gc1 p pp n m =
  let
    divByCoef f = fromJust . pCDiv f 
    ...
    liftToSquare (h,g,hc1,gc1,u,v,pp) = (h',g',hc1',gc1',u^2,v',pp2)
      ...
      pp2  = pp^2
      hc1' = 
           remByCoef pp2 $ upolPseudoRem (hc1+(cPMul pp (u*hc1))) h1
                                              --substitute u(gc1..)?
      gc1' =  
           remByCoef pp2 $ upolPseudoRem (gc1+(cPMul pp (u*gc1))) g1
    ...
    repeatLiftToSquare n (h,g,hc1,gc1,u,v,pp) =
      if n >= m  then  (h,g,hc1,gc1,pp,n)
        ...
        repeatLiftToSquare (2*n) $ liftToSquare (h,g,hc1,gc1,u,v,pp)
    degmessg = "0 < deg h < deg f  required\n"
  in
  case  (p==zr || (eucNorm p)==minNorm,  any isZero [f,h1])
  of
        ...
        (_    , df, dh) -> 
          if  df <= dh || dh <= 0  then  error $ msg degmessg
            ...
            of
              ...
              _            -> 
                       repeatLiftToSquare n (h1,g1,hc1,gc1,u1,v1,pp)
-}
...
testFactorUPol_finField f =
                          tf (zeroS $ sample f) (dom f) (canAssoc f) 
  where
  tf k0 dK cf = case  osetList $ snd $ baseSet k0 dK  of

      Nothing    -> error $ ("testFactorUPol_finField f,"++) $
...
smallInLattice_triang_ ::
    (Field fF) => 
                 UPol fF -> Z -> Z -> [[UPol fF]] -> Maybe [UPol fF]
                 -- pp      l1   bound mH
  -- LOCAL.
...
smallInLattice_triang_  pp l1 bnd mH = 
  let
    ...
    maxsH = map (vecLDeg 0)$ transpose mH  --max order for each col.
    bnsR  = map boundFor1 maxsH
                   ...
                   boundFor1 max1 = let  bn' = bnd + max1 - opp
                                    in
                                    if  bn' < 0  then  -1  else  bn'
    maxBn = maximum (bnd:bnsR)
    psL   = [(i,bnd) | i <- [1..ml']]
    psR   = filter ((>=0).snd) $ zip [(ml'+1)..m1] bnsR
    ps    = psL++psR
    inds  = concat [[[i,j] | j <- [0..d]] | (i,d) <- ps]
                                   --variables (multiindices) 
                                   --the equations to be imposed for
    UPol _ a v dF     = pp
    (zr,un)           = (zeroS a, unity a)
    vcp [i,j] [i',j'] =      -- variable comparison for U = F[uij..]
                             -- - first - by j
                        case 
                             compare j j'  of  EQ -> compare i i'
                                               x  -> x
    ...
    tUpol (i,d) =      -- u(i) = sum [u(i,j)*t^j| j<-[0..d]  of U[t]
          if  
            d < 0  then  zrTU  else  sum1 $ zipWith cPMul cs tPowers
                         where
                         cs = [varToRPol unU un [i,j] | j <- [0..d]]

    uVecL = [tUpol (i,bnd) | i <- [1..ml']]       -- [u_1..u_ml']
    uVecR = map tUpol $ zip [(ml'+1)..m1] bnsR    -- rest  u_i
    ...
    uValuesToA pairs = 
               reverse $ pToVec m1 $ UPol (toOverA pairs) pp "x" eFM
      where
      toOverA []     = []
      toOverA (p:ps) =
                  -- [(uij,aij)| j<-[0..]]-> (sum [aij*t^j...], i-1)
        let  (i:_, _  )     = p
    ...
    of
      ...
      Just pairs ->        -- pairs = [(uij,aij)|..]
        let
          ...
          inds'   = map fst ps
          wrap ps = 
                let (rowL,rowR) = genericSplitAt ml' $ uValuesToA ps
...
linRPolsToStaircase fs =
  ...
  of
    ...
    (g:gs, _  ) -> red (g:gs)
      ...
      red [f] = [f]
      red fs  = 
        let v            = maxBy vcp$ map (fromJust .rpolHeadVar) fs
...
solveLinRPolsStaircase (f:fs) = 
  ...
  solve (f:fs) = -- here fs does not contain constants
    ...
    if  un == (unity un)
      ...
      error $ ("solveLinRPolsStaircase fs  "++)                    $
              ("\n- probably, called from  "++)                    $
                                  ("factorOverFinField_2 q: \n"++) $
              ("\n\nnot normed r-polynomial met in the system"++)  $
                                                  (" - why ?\n"++) $
              ("\nlength fs =  "++) $ shows fsL                    $
