...
import DPrelude   (Z, Comparison, eFM, maximum, ct, ctr, allMaybes,
                   showsWithDom
                  )
...
import Pol_       (Pol(..), polMons, headVarPol, fromHeadVarPol, 
                   reordPol, polDegs, toUPol, fromUPol, 
                   polPermuteVars
                  )
import Pol1_      (polSubst                                        )
import Pol2_      (toOverHeadVar, fromOverHeadVar                  )
-- import RdLatP_    (reduceLattice_UPolField                         )
import Pgcd_      (upolGCD_Chinese                                 )
import Pfact0_    (henselLift, smallInLattice_triang_              )
import Pfact__    (factorUPol_finField {- ,FactorizationRing UPol-})
        ...
        if  any (/=0) rems  then
          ...
          let
            ...
            factorFurther ((True ,h),j) = 
                   [(p,i*j*q)| (p,i)<- factorPrimitive $ canAssoc h]
          in
  ...
  factorSquareFree f =  
    ...
    let                 
      ...
      fRfactn   =                     --Step f: factoring over A/(p)
        if  oP > 1  
          then  map fst $ factorUPol_finField fR 
        else 
