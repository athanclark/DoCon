...
import DPrelude  
       (MMaybe, PropValue(..), Expression(..), InfUnn(..), Z, lkp, 
        eFM, showsExpr, allMaybes, sum1, ct, ctr, tuple31, tuple32,
        tuple33
       )
import Categs
import SetGroup 
...
import UPol_     (PolLike(..), lc, lc0, upolMons, ppoComp, lexPPO, 
                  cPMul
                 )
...
instance (CommutativeRing a, Random a) => Random (Pol a)  
  ...
  randomR (l,h) g = 
    let  
      ...
      mergeMons mons         []            = 
                                         [(a,zr,p) | (a,p) <- mons ]
      mergeMons ((a,p):mons) ((b,q):mons') = case  cp p q  of
...
set_ pDomString dom aD sampleP bel =      -- reused in  RPol, SymPol
  (case  
       (lkp dom Set, lkp aD AddSemigroup)
   of
  ...
  pset Yes = (addToFM dom Set $ D1Set s, s)
...
asmg_ pDomString dom aD zeroP =          -- re-used in  RPol, SymPol
  (case
       (lkp dom AddSemigroup, lkp aD AddSemigroup)
   of
  ...
  semig aS = case  isGroup aS  of
                      Yes -> (addToFM dom AddSemigroup $ D1Smg s, s)
...
agr_ pDomString dom aD zeroP =           -- reused for  RPol, SymPol
  (case
       (lkp dom AddGroup, lkp aD AddSemigroup)
   of
    ...
    gr aS = case  isGroup aS  of
                        Yes -> (addToFM dom AddGroup $ D1Group g, g)
                        _   -> (dom, error $ msg msg')
    g = Subgroup 
...
msmg_ pDomString dom aD unpM = case  lkp dom MulSemigroup  of

  Just (D1Smg s) -> (dom, s)
  _              ->
    (case  catMaybes [lkp aD AddSemigroup, lkp aD MulSemigroup]
     of
       [D1Smg aAS, D1Smg aMS] ->
              ...
              Just p  -> (pp',ppInv')
                ...
                ppInv' g = 
                  ...
                  if  any (not .isZeroPP) rs  then  Nothing  
                    ...
                    of
                      Nothing  -> Nothing
                      Just cs' -> case  allMaybes cs'  of

                           Just bs -> Just $ Just $ ct f $ zip bs qs
                           _       -> Nothing 
...
polSubst  mode  f@(Pol _ _ o vars dA)  gs  powerLists =    
  let
    ...
    subst (g:gs) pss vars' (v:vs) f = case  (pIsConst f, vs)  of
      ...
      _          -> 
        let  (cs,js) = unzip $ upolMons $ headVarPol eFM f
                                              --here ci <- R[x2..xn]
             powersG = powers js pss g     
  ...
  if  mode=='l'  then  subst gs pss [] vars f
    ...
    reordPol o $ subst gs pss [] vars $ reordPol o' f
  ...
  (u,vars) -> rd e
    ...
    rd e = case  fromExpr u e  of   -- first try coefficient
      ...
      _        -> 
        ...
        variable _ (vs,vs') = let ns  = (map (const 0) vs) ::[Z]
