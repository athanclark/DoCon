...
module Partition  
  ...
  --
  -- This is the head module which also reexports all the needed
  -- items from  Partit_, HookBand_.
...
import Data.FiniteMap (FiniteMap, lookupFM, addToFM)

import List     (genericLength, genericReplicate)
import DPrelude (Z, eFM, factorial, mergeBy     )
import Partit_  
import HookBand_ 
...
{-    
Number of tableaux of the shape la  and weight  mu, 
- for |la|==|mu|.

`table' = tab(ro) 
is a FiniteMap  (binary table - made functionally)  serving to store
the pairs  
...
accumulated table - even if emptyFM is initiated in the argument.
...
kostkaNumber tab la mu = 
  let
    ...
    kn tab bnd  la       mu@(_:muT) = case  lookupFM tab la  of   
      ...
      _      -> 
        let  laLower = subtrHBand la bnd 
             ...
             (tabLow,nLow) = case  maxHWBand 'l' laLower w'  of
...
--------------------------------------------------------------------
kostkaColumn :: 
            Partition -> [Partition] -> (FiniteMap Partition Z, [Z])
            -- mu        la_s            tab                    col
  -- yields 
...
kostkaColumn mu la_s = 
  if  
    ...
    where
    (tab,col)           = foldl addKNum (eFM,[]) la_s   
    addKNum (tab,vs) la = (tab',v:vs)  where 
                                   (tab',v) = kostkaNumber tab la mu
    msg  = 
        ("length la_s = "++).shows (genericLength la_s).('\n':).msg'

    msg' = case  la_s  of  la:_ -> ("head la_s = "++) .shows la
                           _    -> id
--------------------------------------------------------------------
kostkaTMinor :: [Partition] -> [Partition] -> [[Z]]
kostkaTMinor    la_s           mu_s        = 
                           [snd $ kostkaColumn la mu_s | la <- la_s]
  --
  -- Transposed minor of the matrix of the Kostka numbers.
  -- la_s, mu_s   should be the partitions of same weight w > 0,  
  -- la_s, mu_s   
  -- define respectively the rows and the columns of transposed 
  -- minor. 
...
permGroupCharValue tab la ro = case  (la,ro)  of
  ...
  _               -> cv tab la $ reverse $ toEPrtt ro
    ...
    cv tab la      (w:ro) = 
      let
         la'tab = lookupFM tab $ conjPrtt la
      in 
      case  (firstSWHook w la, lookupFM tab la, la'tab)
      of  
        (Nothing, _     , _     ) -> (tab, 0)
        (_      , Just v, _     ) -> (tab, v)
    ...
    permutSign pt =  
          if  even (genericLength $ filter even pt)  then 1  else -1
    ...
    runHooks tab la (w:ro) hook totalSum =  
       ...
       of
         Nothing    -> (addToFM tab' la newSum, newSum)
...
permGroupCharColumn ro la_s   = (tab, reverse col)
    where
    (tab,col) = foldl addCharValue (eFM,[]) la_s
...
permGroupCharTMinor    ro_s           la_s =
  ...
  --
  -- ro_s, la_s   should be the partitions of same weight w > 0,  
  -- they define respectively the rows and columns of transposed 
  -- minor. 
  -- In particular, setting  ro_s = la_s = all_partitions(w) 
  -- yields the whole transposed character matrix  tC.
  --
  -- One of TESTs:  tC * (transp tC)  should be diagonal
