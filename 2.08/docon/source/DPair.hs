...
import DPrelude  (eFM, lookupProp, and3, lkp, showsWithDom)
import SetGroup    
...
instance (Set a, Set b) => Set (a,b)      -- baseSet(a) x baseSet(b)
  ...
  showsDomOf (a,b) = 
               ('(':) .showsDomOf a .(" x "++) .showsDomOf b .(')':)

  baseSet (x,y) dom =  
               case  (lkp dom Set, baseSet x eFM, baseSet y eFM)  of
...
instance (AddSemigroup a, AddSemigroup b) => AddSemigroup (a,b) 
  ...
  baseAddSemigroup (x,y) dom =  
    case  
        (lkp dom AddSemigroup, baseAddSemigroup x eFM,
                                              baseAddSemigroup y eFM
        )
    of
...
instance (AddGroup a, AddGroup b) => AddGroup (a,b)   
  ...
  baseAddGroup (x,y) dom =  
    case  
        (lkp dom AddGroup, baseSet x eFM, baseSet y eFM)
    of
...
instance (MulSemigroup a, MulSemigroup b) => MulSemigroup (a,b) 
  ...
  divide_m2 _ _ = error ("divide_m2 (x,y) (x',y'):  \n"++
                         "not defined for Pair, so far, sorry\n" 
                        )
  power_m (x,y) n = maybePair (power_m  x n) (power_m  y n)

  root n (x,y) = case  (root n x, root n y)  of
  ...
  baseMulSemigroup (x,y) dom =  
    case  
        (lkp dom MulSemigroup, baseMulSemigroup x eFM,
                                              baseMulSemigroup y eFM
        )
    of
      (Just (D1Smg s), _    , _      ) -> (dom, s)    
      (_             , (_,s1), (_,s2)) -> 
                             (addToFM dom MulSemigroup (D1Smg s), s)
                                   where
                                   s = directProduct_semigroup s1 s2
...
instance (MulGroup a, MulGroup b) => MulGroup (a,b) 
  ...
  baseMulGroup (x,y) dom =  
    case  
        (lkp dom MulGroup, baseSet x eFM, baseSet y eFM)
    of
      (Just (D1Group g), _        , _        ) -> (dom, g)    
      (_               , (dom1,s1), (dom2,s2)) -> 
        let
        ...
        in
        (addToFM dom MulGroup (D1Group g), g)
...
instance (Ring a, Ring b) => Num (a,b)  
  ...
  signum _ = error ("signum (_,_):  not defined for Pair\n")
  abs    _ = error ("abs (_,_):  not defined for Pair\n")
  fromInteger _ = error "fromInteger (_,_):  use  fromi, fromi_m\n"
...
instance (Ring a, Num a, Ring b, Num b) => Fractional (a,b)  
  ...
  fromRational _ = 
      error "fromRational:  use  fromi_m  composed with  divide_m\n"
...
instance (Ring a, Ring b) => Ring (a,b)  
  ...
  baseRing (x,y) dom =  
    case 
        (lkp dom Ring, baseRing x eFM, baseRing y eFM)
    of
...
instance (LinSolvRing a, LinSolvRing b) => LinSolvRing (a,b)
  ...
  baseLinSolvRing (x,y) dom =
    case
        (lkp dom LinSolvRing, baseLinSolvRing x eFM, 
                                               baseLinSolvRing y eFM
        )
    of
      ...
      (_                  , (_,t1), (_,t2)) ->
        let
          ...
          props =
                [(ModuloBasisDetaching, and3 aModDetach bModDetach),
        ...
        in
        (addToFM dom LinSolvRing (D1LinSolvR t), t)
  ...
  moduloBasis mode pairs (a,b) =    -- String -> [a] -> a -> (a,[a])
    let
      ...
      msg = ("moduloBasis "++) .(mode++) .(" pairs pair,\n"++) 
            .showsWithDom (a,b) "pair" ""
    in
  ...
  syzygyGens mode []    = error $ ("syzygyGens "++) $ 
                                        (mode++" []    for (a,b)\n")
  syzygyGens _    pairs =     
    let
      (zrA,zrB)     = zeroS $ head pairs
